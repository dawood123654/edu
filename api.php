...
function validate_required(array $data, array $fields): void
{
  ...
  if ($missing)
    json_error('Validation failed', 422, ['missing' => $missing]);
}
...
try {
  switch ($action) {
    ...
    case 'register':
      ...
      if ($st->fetch())
        json_error('Email already registered', 422, ['email' => 'taken']);
    ...
    case 'save_attempt':
      ...
      if (!is_array($answers))
        json_error('Answers must be an object/array', 422);
    ...
    case 'list_attempts':
      ...
      $rows = $st->fetchAll();
      foreach ($rows as &$r) {
        $r['answers'] = $r['answers_json'] ? json_decode($r['answers_json'], true) : [];
    ...
    case 'ai_suggest_major':
      ...
      $certificateBase64 = (string) ($input['certificate_base64'] ?? '');
      save_certificate_image($u['id'], $certificateBase64);
      $major = ai_recommend_major($gpa, $gat, $tahsili, $subjects, $certificateBase64);
...
function save_certificate_image(int $userId, string $base64): ?string
{
  if (!$base64)
    return null;
  $clean = normalize_base64_input($base64);
  if (!$clean)
    return null;
...
function normalize_base64_input(string $base64): string
{
  if (!$base64)
    return '';
...
function ai_recommend_major(float $gpa, float $gat, float $tahsili, array $subjects, ?string $certificateBase64): string
{
  $apiKey = "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"
  ...
  $cleanCert = normalize_base64_input($certificateBase64 ?? '');
  $certNote = $cleanCert ? 'الشهادة مرفقة، اقرأ الدرجات الظاهرة فيها إن وجدت.' : 'لا توجد صورة شهادة مرفقة، اعتمد على الأرقام المدخلة فقط.';

  $prompt = "أنت مستشار قبول جامعي سعودي دقيق. لديك درجات الطالب وصورة شهادة ثانوية (إن توفرت). اقرأ الأرقام من الشهادة عند توفرها، وإن تعارضت مع القيم المرسلة فاعتمد ما هو أوضح في الشهادة. النتيجة المطلوبة: اسم تخصص واحد فقط باللغة الإنجليزية، كلمة أو كلمتين، بدون أي شرح أو JSON. التزم بقائمة التخصصات الشائعة: General Doctor, Dentistry, Pharmacy, Mechanical Engineer, Electrical Engineer, Civil Engineer, Computer Science, Cybersecurity, Data Science, Nursing, Business Administration, Finance, Accounting, Law, Sharia, English Literature, Design, Interior Design. إذا كان الطالب مناسباً للطب فأجب General Doctor. إذا كانت الدرجات ضعيفة فاختر Humanities.\n" .
    "البيانات المدخلة:\n- GPA من المدخلات: {$gpa}\n- قدرات: {$gat}\n- تحصيلي: {$tahsili}\n- درجات المواد (نصية): {$subjectText}\n- ملاحظة الشهادة: {$certNote}\n" .
    "$guidelines\n" .
    "أجب الآن بتخصص واحد فقط مثل Doctor أو Pharmacy أو Computer-Science بلا أي نص آخر.";
  ...
  if ($cleanCert) {
    $userContent[] = [
      'type' => 'image_url',
      'image_url' => ['url' => 'data:image/png;base64,' . $cleanCert]
    ];
  }
  ...
  $body = [
    ...
    'messages' => [
      ['role' => 'system', 'content' => 'انت خبير قبول جامعي سعودي صارم بالنسب وتقرأ درجات الشهادة المرفقة إن وجدت. الجواب كلمة تخصص واحدة فقط بالإنجليزية بدون شرح.'],
